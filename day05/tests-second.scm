...
(define stream-of-lines (@@ (second) stream-of-lines))
(define input-string
         "0,9 -> 5,9
8,0 -> 0,8
9,4 -> 3,4
...
(define make-vent-line (@@ (second) make-vent-line))
(define vent-horiz-or-vert? (@@ (second) vent-horiz-or-vert?))
(define direction (@@ (second) direction))
(define line->vent-line (@@ (second) line->vent-line))
...
;; import internal functions of module to test
(define vent-line->path (@@ (second) vent-line->path))
...
(define input-strm->overlap-count (@@ (second) input-strm->overlap-count))

(test-begin "vent grid overlaps")
(test-equal 12 (input-strm->overlap-count input-strm))
